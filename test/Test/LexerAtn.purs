-- | Phase 4 gate: the ATN-driven lexer (`Gramark.Lexer.Atn`, the
-- | `LexerATNSimulator` port) tokenizes **identically** to the production
-- | regex-DFA scanner (`Gramark.Scanner`) on the capture-free corpus — every
...
import Gramark.Conformance.Lexers (grammarLiterals, tokensBlock)
import Gramark.Lexer (Token)
import Gramark.Lexer.Atn (buildLexerAtn, runLexerAtn)
import Gramark.Lr (parse)
import Gramark.Scanner (buildItems, scan)
...
tests = do
  ...
  jsonMd <- readTextFile UTF8 "examples/json.grmk.md"
