...
**Target repo:** `wstein/gramark`, branch `develop`. Stack: **Astro 7 + Starlight 0.41**, base
path `/gramark/`, deployed to GitHub Pages.
**Design reference:** `Gramark Site.dc.html` (this bundle) + the token/brand spec in `README.md`.

> ⚠️ **Read this first — the single most important rule.**
> The prototype (`Gramark Site.dc.html`) contains a **stand‑in JavaScript grammar engine** (lexer,
> Earley parser, FIRST/FOLLOW, forest, evaluator). **Do NOT port it.** The repo already has the
> **real engine** — the compiled PureScript core `Gramark.Playground`, bundled to
> `site/src/generated/gramark-engine.mjs` and wrapped by `site/src/lib/gramark-runtime.ts`
> (`parseGramarkDocument`). It is the same code the `gramark` CLI runs, so browser and CLI can never
...
`parseGramarkDocument(source, input)` → `GramarkParseResult` (see
`site/src/generated/gramark-engine.d.ts`). Fields, and which mock tab each feeds:
...
- **FIRST/FOLLOW sets** — not in the result. Either (a) add to `Gramark.Playground.Result` and
...
- **`site/public/favicon.svg`** — replace with the refined railroad logomark (96×96, spec + exact
  path data in `README.md` → *Logomark*; reference component `GramarkMark.dc.html`). Ensure it reads
...
- **`brand/gramark-logomark.svg` / `gramark-wordmark.svg` / `gramark-wordmark-dark.svg`** — regenerate
...
- **Landing — `site/src/content/docs/index.mdx`** (or convert to a custom `src/pages/index.astro`
  ...
  **splash** template (`template: splash` in frontmatter) and build the hero + live showcase +
  ...
  - Sub: the `.grmk.md`-is-both-doc-and-input paragraph.
...
- **Tutorial — `site/src/content/docs/tutorials/intro.mdx`** (already 10 KB of real content). Keep
  ...
  `parseGramarkDocument` + `renderDiagrams`.
...
real engine calls that are already wired** (`parseGramarkDocument`, `renderDiagrams`, and the
...
4. **Status strip**: full-width `--out-bg`, mono — accept/reject, `METHOD(1) · start X`,
   `gramark --check · clean | N conflicts` (N from `conflicts`).
...
variant. All variants call the same `parseGramarkDocument` + `renderDiagrams`.
...
These require touching the PureScript `Gramark.Playground` module and regenerating the bundle
(`npm run build:engine`, which runs `spago build` + esbuild → `src/generated/gramark-engine.mjs`;
keep `gramark-engine.d.ts` in sync). Prioritized:
...
- [ ] `npm run lint` / `prettier` clean; `npm run build` (Astro) and `npm test`
      (`gramark-runtime.test.ts`) pass; if the engine was touched, `npm run check:engine` is clean.
...
brand/gramark-*.svg                        REGENERATE: refined mark + split-stem wordmark
...
site/src/lib/gramark-runtime.ts           KEEP (engine wrapper) — maybe widen result if §5
site/src/lib/diagrams.ts                  KEEP (railroad renderer)
site/src/generated/gramark-engine.{mjs,d.ts}  regenerate ONLY if §5 extensions
