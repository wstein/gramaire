...
import gramark.{Playground, PlaygroundResult}

// The browser entry point (ADR D13, Phase 1.8): a thin `@JSExportTopLevel`
// wrapper around the real, filesystem-free `gramark.Playground.evaluate`.
// Reproduces the exact JS contract the site's `gramark-runtime.ts` expects
