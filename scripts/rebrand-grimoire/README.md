# Gramark → Grimoire rebrand mirror script

Renames `.grmk`/`.grmk.md` → `.gram`/`.gram.md` and rebrands `Gramark`/`gramark` →
`Grimoire`/`grimoire` across an entire git history, via `git-filter-repo`, operating only on a
...
- `design/gramark-site-handoff/` is documented (`docs/rebrand-grimoire-plan.md`) as a frozen
  ...
  - Caught by testing: the global rebrand also touches every OTHER file's TEXT MENTIONS of that
    ...
    `design/grimoire-site-handoff/*.dc.html`, a rule pointing at a directory that (deliberately)
    no longer exists under that name. `replace-text-rules.txt`'s last rule
    (`grimoire-site-handoff==>gramark-site-handoff`) reverts just that one derived path fragment
...
- Rename the GitHub repository itself, or migrate the GitHub Pages URL — both are separate,
  sequenced decisions per `docs/rebrand-grimoire-plan.md` (Pages project-site URLs don't
...
- Update the brand assets (logomark/wordmark) — those need an actual redesign pass, not a
  mechanical rename (see `docs/rebrand-grimoire-plan.md`'s sequencing: brand design comes first).
