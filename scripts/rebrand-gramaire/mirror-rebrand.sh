...
# OLD target name before the 2026-07-05 collision-risk pivot), rename the .gram/.gram.md file
...
#
# Example:
#   ./mirror-rebrand.sh git@github.com:wstein/gramaire.git ./gramaire-mirror-work
...
SOURCE_URL="$1"
WORK_DIR="${2:-$SCRIPT_DIR/gramaire-rebrand-work}"
MIRROR_DIR="$WORK_DIR/gramaire-mirror.git"
...
mkdir -p "$WORK_DIR"
FILE_INFO_CALLBACK_FILE="$WORK_DIR/file_info_callback.py"
cat > "$FILE_INFO_CALLBACK_FILE" <<PY
import importlib.util

spec = importlib.util.spec_from_file_location('file_info_callback', r'$SCRIPT_DIR/file_info_callback.py')
file_info_callback = importlib.util.module_from_spec(spec)
spec.loader.exec_module(file_info_callback)

return file_info_callback.file_info_callback(filename, mode, blob_id, value)
PY
...
GRAMARK_HITS="$WORK_DIR/sanity-gramaire-hits.txt"
git -C "$REVIEW_CHECKOUT_DIR" grep -ilE 'gramaire' -- . >"$GRAMARK_HITS.raw" 2>/dev/null || true
...
GRIMOIRE_HITS="$WORK_DIR/sanity-gramaire-hits.txt"
git -C "$REVIEW_CHECKOUT_DIR" grep -ilE 'gramaire' -- . >"$GRIMOIRE_HITS" 2>/dev/null || true
show_first_or_none "$GRIMOIRE_HITS"
echo "-- remaining .gram/.gram.md/.gram.lock paths, if any:"
GRMK_PATH_HITS="$WORK_DIR/sanity-gram-path-hits.txt"
git -C "$REVIEW_CHECKOUT_DIR" ls-files | grep -F '.gram' >"$GRMK_PATH_HITS" || true
...
original wstein/gramaire -- that's a separate, much bigger decision this script does
