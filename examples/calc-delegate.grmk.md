...
The `calc-js` calculator (`examples/calc-js.grmk.md`), extended with a few
...
and `{%? %}` in `examples/predicate-guard.grmk.md`). Three constructs use a
delegate instead:
...
- A bare function call, `IDENT '(' Expr ')'`, delegates to `Call` with **no**
  ...
  unresolved, so `gramark emit --backend js`'s output only works once a
