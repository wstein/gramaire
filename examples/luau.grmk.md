...
Gramark from the language's own published sources.
...
that Gramark's `X*`/`X?` postfix only covers for a single symbol, and folds
`(…)` alternation groups inline. As in the ANTLR v4 conversion
([`examples/antlr/antlr4.grmk.md`](antlr/antlr4.grmk.md)), each `{X Y}`/`[X Y]`
...
Constructs Gramark cannot model as a token regex are **flagged, not silently
dropped** — see
...
a same-shaped regex match — see `examples/predicate-guard.grmk.md`), so no
...
optionally `;`-terminated). Gramark's Core is epsilon-free (an alternative
...
`examples/calc-prec.grmk.md` resolves its own natural-ambiguous form with a
`## Precedence` block. Gramark's LR precedence declarations only cover
...
`binop`, and Gramark has no yacc-style per-alternative `%prec` override yet);
...
how `examples/calc.grmk.md` stratifies into `Expr`/`Term`/`Factor` instead of
...
`{attribute}` — zero or more attributes. As with `block` above, Gramark's
...
arbitrary-length balanced `Type`, so `gramark emit` genuinely can't build
...
through `gramark emit` (canonical LR(1)): building `{'|' X}`/`{'&' X}` via
Gramark's `*`/`+` EBNF sugar — which lowers by enumerating present/absent
...
`gramark emit` (`shift ')' vs reduce TypeList -> Type` — the reducing
...
These Luau lexical features have no equivalent in Gramark's regular-language
...
- **`--[[`/`--` priority.** `LINE_COMMENT`'s regex above excludes the
  ...
  repo (e.g. `examples/antlr/antlr4.grmk.md`'s "DFA-friendly, no
...
### Known ambiguities `gramark emit` cannot resolve

`gramark check`/`fmt` (this file's actual CI gates — structure and drift,
never a full LR(1) build) pass cleanly; `gramark emit`'s default `lr`
strategy does not fully build this grammar, the same documented status
`examples/dangling-else.grmk.md` has. Three genuine ambiguities remain,
