...
-- |
-- | A `Gramark.Table.Conflict` carries the competing production indices, but
...
import Gramark.Table (Conflict(..), GSym(..), Prod, nontermSet, productions)
...
-- | phantom terminal the way `Gramark.Table.resolve` would otherwise treat it.
