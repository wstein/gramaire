...
module Gramark.Conformance.Lexers
  ( Lexer
  , lrLexer
  , calcLexer
  , scannerLexer
...
-- | Extract the content of the first ```` ```gramark tokens ```` block from a
...
tokensBlock md0 =
  ...
  step acc line
    ...
    | trim line == "```gramark tokens" = acc { inside = true, cur = [] }
    | otherwise = acc
