-- | A `format` backend: `gramark-ir` in, an ANTLR4 `.g4` grammar out.
-- |
-- | This is the **tractable half** of the ANTLR ↔ Gramark converter (the
-- | ALL(\*) port, Phase 3, D-strategy): Gramark → ANTLR is a structural
-- | projection, because Gramark's Core is a subset of what ANTLR expresses. The
...
-- |     translated from Gramark's regex sublanguage to ANTLR lexer notation
-- |     (`(?:…)` → `(…)`, `[^…]` → `~[…]`, escaped and bare literals quoted),
-- |     with `%skip` rendered as `-> skip`.
-- |
-- | The reverse direction (ANTLR → Gramark) is the hard half — semantic
-- | predicates, actions, and lexer modes have no Core equivalent — and is out of
-- | scope here.
module Gramark.Backend.Antlr
  ( backend
  , emit
  , regexToAntlr
  ) where
...
import Gramark.IR (IR, IRLexer, IRPattern(..), IRRef(..), IRRule, IRTerminal(..), IRTokenClass)
...
-- | Translate Gramark's restricted regex sublanguage (lexer-spec §10) to ANTLR4
