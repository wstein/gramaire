...
-- | and `buildTables` produces the parse tables `Gramark.Parser` runs.
-- |
-- | The grammar is assumed epsilon-free (Gramark grammars enumerate
...
-- | Parse a `## Precedence` block's content (its `gramark precedence` lines)
...
parsePrecedence content =
  ...
  where
  meaningful =
    Array.filter (\l -> l /= "" && l /= "gramark precedence")
...
resolve _ Any = Term "(any)" -- unreachable: `.` is lowered before table construction
resolve _ (Not _) = Term "(not)" -- unreachable: `~` is lowered before table construction
