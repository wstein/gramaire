-- | Lower a **desugared** `Gramark.Syntax.Grammar` into an `Atn` (the ALL(*)
...
-- |
-- | The input MUST be desugared (`Gramark.Desugar`): `X+`/`X*`/`X?`/macros are
...
-- | follow.
module Gramark.Atn.Build (buildAtn) where
...
import Gramark.Atn (ATNState, Atn, StateKind(..), Transition(..))
