...
-- | `Gramark.Atn`, so a backend can ship the network instead of LR tables. Only
...
-- | `Gramark.Ll` does — is serialized into the IR so a backend can ship it
...
-- | host language (`Gramark.Lr.actionLangOf`). `Nothing` leaves the IR
