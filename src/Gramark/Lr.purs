...
import Gramark.Bootstrap (bootstrapGrammar, lrTokensSource)
import Gramark.Desugar (desugar)
import Gramark.Diagnostics (checkDefined)
import Gramark.Lexer (Token, normalizeNewlines)
import Gramark.Scanner (ScanItem, buildItems, hasError, scan)
import Gramark.Tokens (parseTokens)
import Gramark.Parser (run)
import Gramark.Syntax (Alt(..), Grammar(..), Rule(..), Sym(..))
import Gramark.Table (Method(..), Precedence, buildTablesFor, emptyPrec, parsePrecedence)
...
-- | (the order `Gramark.Table.productions` flattens `bootstrapGrammar` into).
-- | This is the artifact `gramark fmt` codegen will emit; for now it is
...
-- | `gramark precedence` / `gramark errors` — from a `.grmk.md` document, in order.
...
lrBlocks md =
  ...
  scan acc line =
    ...
    else if trim line == "```gramark" then acc { inside = true, cur = [] }
    else acc

-- | Every ` ```gramark `* fenced block with its info suffix — `""` for the
-- | production blocks, `"tokens"` / `"precedence"` / `"errors"` for the sidecars
-- | — in document order.
gramarkBlocks :: String -> Array { info :: String, content :: String }
gramarkBlocks md =
  ...
  step acc line =
    ...
    else case stripPrefix (Pattern "```gramark") (trim line) of
      Just rest -> acc { inside = true, info = trim rest, cur = [] }
      Nothing -> acc

-- | The raw `.grmk` projection (ADR D36): a FENCE-FREE, marker-free export that
...
-- | the two are intermixed and told apart **by case** — exactly how Gramark
...
-- | only sections that carry a `gramark` / `gramark tokens` / `gramark
...
strip md =
  ...
  -- A `## ` section survives only if it carries a keepable gramark block; its
  ...
  walk acc line =
    ...
    in
      case acc.keep of
        ...
        Nothing -> case stripPrefix (Pattern "```gramark") t of
  ...
  keepableOpen line = case stripPrefix (Pattern "```gramark") (trim line) of
...
toFenced src =
  if contains (Pattern "```gramark") src then src
  else
    let
...
-- productions and re-fenced into its own `gramark settings` block.
...
-- | the implicit literals. The hand-written `Gramark.Lexer` is now only the
...
precedenceOf md = case Array.find (\b -> b.info == "precedence") (gramarkBlocks md) of
  Just b -> parsePrecedence b.content
  Nothing -> emptyPrec

-- | The declared inline-action host language of a `.grmk.md` — its `## General
