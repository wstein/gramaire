...
-- | eats it natively, but `Gramark.Ll` would not terminate. This pass rewrites
...
-- |
-- | It runs **after** `Gramark.Desugar`, so the left-recursive list rules that
...
-- | (the corpus has none).
module Gramark.LeftRec (eliminate) where
