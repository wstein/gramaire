...
-- |
-- | `run` is the generic shift/reduce driver every Gramark-generated parser
...
-- |     actions live.
-- |
-- | `Gramark.Lr` instantiates it for the `lr` notation itself.
module Gramark.Parser
  ( ParseError(..)
  , run
  ) where
...
import Gramark.Table (Action(..), GSym(..), ParseTable)
