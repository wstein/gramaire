...
class BackendJsSuite extends munit.FunSuite:
  ...
  // For a hand-built `Grammar` (no `.grmk` text, so no `lang:` to read) — the action-lang key
  ...
  // For real `.grmk.md` text using a `-> name`/`-> name(args)` delegate (ADR D48) and, optionally,
  // a `## Externals` section (ADR D49) — mirrors the real `gramark emit --backend js` pipeline
  ...
  // backend still silently drops both (`GramarkLint.delegateLoss`/`externalsLoss`).
  ...
  test(
    ...
    // examples/calc-delegate.grmk.md's `Call` delegate, caught — BackendJsExternalsExecSuite).
