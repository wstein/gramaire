...
// Ported from src/Gramark/Playground.purs.
final case class PlaygroundResult(
    ...
    allCstJson: Vector[
      String
    ], // every derivation as gramark-cst JSON; >1 entry only when ambiguous
    prodLhs: Vector[
      String
...
object Playground:
        ...
        lexer(input) match
          ...
          case Right(toks) =>
            ...
            PlaygroundResult(
              ...
              diagnostics =
                if accepted then Vector("Accepted by the Gramark engine.")
  ...
  /** The grammar's self-contained JS evaluator (`Gramark.Backend.Js`): its inline `{% %}` actions
