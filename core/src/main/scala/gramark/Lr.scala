...
object Lr:
  ...
  /** `toFenced` plus whether it actually changed anything — i.e. whether `src` was a fence-free
    * `.grmk` projection that got lossily reconstructed (reordered, comments stripped), as opposed
    * to an already-fenced `.grmk.md` passed through unchanged. Callers that report a diagnostic's
  ...
  // `private[gramark]`, not `private`: `BackendJs` reuses this exact fold to match a `## Externals`
  ...
  private[gramark] def normalizeLang(raw: String): String =
