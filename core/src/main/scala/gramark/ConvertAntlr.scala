...
// The **import half** of the ANTLR <-> Gramark converter: an ANTLR4
// `.g4` grammar in, a Gramark `.grmk.md` document out. Keeps what has a
...
object ConvertAntlr:
  ...
  private def renderAtom(a: Atom): String = a match
    ...
    // `renderTopAlt`, since Gramark's action slot is trailing-only and one-per-alt, not a
  ...
  // Gramark's action slot is one-per-alt and trailing-only, and an action-only alt would be an
  ...
  /** Import an ANTLR4 `.g4` grammar, producing a rendered `.grmk.md` and any features that could
    * not be represented.
    */
