...
// A `format` backend: `gramark-ir` in, an ANTLR4 `.g4` grammar out. The
// tractable half of the ANTLR <-> Gramark converter — each nonterminal
// becomes a parser rule, each token class a lexer rule. The reverse
// direction (ANTLR -> Gramark, ConvertAntlr) is the hard half.
// Ported from src/Gramark/Backend/Antlr.purs.
object BackendAntlr:
  ...
  /** Translate Gramark's restricted regex sublanguage to ANTLR4 lexer notation.
    */
