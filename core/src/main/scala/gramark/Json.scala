...
//
// Gramark emits its narrow-waist artifact (`gramark-ir`) as JSON, and the
...
// `Map`.
// Ported from src/Gramark/Json.purs.
enum Json derives CanEqual:
  case JNull
  case JBool(b: Boolean)
  case JInt(n: Int) // the only number form Gramark needs — every numeric IR field is an integer
