...
// `gramaire-ir` plus the `lr` typed-AST profile (the per-symbol `SemVal`
...
object CodegenScala:
  ...
  val lrActionsScala: Map[Int, String] = Map(
    ...
    37 -> "(_, name) => Some(DelegateSpec(name, Vector.empty))", // Delegate : ARROW IDENT
    38 -> "(_, name, _, args, _) => Some(DelegateSpec(name, args))", // Delegate : ARROW IDENT '(' ArgList ')'
    39 -> "(a) => Vector(a)", // ArgList : Arg
    40 -> "(args2, _, a) => args2 :+ a", // ArgList : ArgList COMMA Arg
    41 -> "(i) => i", // Arg : IDENT
    42 -> "(t) => t", // Arg : TERM_LIT
    43 -> "(n) => n", // Arg : NUMBER
    44 -> "(syms) => Vector(syms)", // GroupBody : SymList
    45 -> "(alts, _, syms) => alts :+ syms", // GroupBody : GroupBody '|' SymList
    46 -> "(_) => Any", // Atom : '.'
    47 -> "(_, s) => Not(s)", // Atom : '~' NotArg
    48 -> "(i) => Vector(i)", // NotArg : SetItem
    49 -> "(_, s, _) => s", // NotArg : '(' SetBody ')'
    50 -> "(i) => Vector(i)", // SetBody : SetItem
    51 -> "(s, _, i) => s :+ i", // SetBody : SetBody '|' SetItem
    52 -> "(i) => Ref(i)", // SetItem : IDENT
    53 -> "(t) => Lit(t)" // SetItem : TERM_LIT
  )
  ...
  private val header: String =
    Vector(
      ...
      "",
      "import gramaire.Sym.*",
      "import gramaire.{Alt, DelegateSpec, Grammar, Rule, SemVal}",
      "",
