...
// runtime all agree, and `grammar/lr.grmk.md` is the single source of
...
class SelfHostSuite extends munit.FunSuite:
  private val md = java.nio.file.Files.readString(java.nio.file.Path.of("grammar/lr.grmk.md"))

  test("canonical parse(grammar/lr.grmk.md) == bootstrapGrammar") {
