...
// JVM-only (reads examples/*.grmk.md): pins Atn.wellFormed plus rule
...
class AtnInvariantSuite extends munit.FunSuite:
  ...
  test("calc: ATN construction invariants hold over the real grammar") {
    Lr.parse(readFile("examples/calc.grmk.md")) match
  ...
  test("json: ATN construction invariants hold over the real grammar") {
    Lr.parse(readFile("examples/json.grmk.md")) match
  ...
  test("ECMA-404: ATN construction invariants hold over the real grammar") {
    Lr.parse(readFile("examples/ECMA-404.grmk.md")) match
  ...
  test(
    ...
    Lr.parse(readFile("examples/calc-prec.grmk.md")) match
