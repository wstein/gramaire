...
class IRGoldenSuite extends munit.FunSuite:
  ...
  test("grammar/Productions.gram.md -> IR matches the committed golden") {
    golden("grammar/Productions.gram.md", "test/golden/lr.ir.json", "Lr")
  }
  ...
  for (path, name) <- List(
      "grammar/Productions.gram.md" -> "Lr",
