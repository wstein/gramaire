...
// `cli/jvm/src/main/scala/gramark/cli/ScalaPegParityMain.scala`). This file is the ONLY thing in
...
// dependency on `gramark-core` (this subproject exists specifically to prove the emitted Scala
...
object Main:
  ...
  // Mirrors `gramark.Cst.render`/`showString` exactly (core/src/main/scala/gramark/Cst.scala) —
  // duplicated, not imported, since this subproject must not depend on `gramark-core` (see the
  ...
  // real `gramark.Cst.render`), never a silent false pass, so a copy is safe here.
  ...
  def main(args: Array[String]): Unit =
    args match
      case Array(vectorsPath) =>
        ...
        println(endMarker)
      case _ =>
        System.err.println("usage: gramark.scratch.Main <vectorsPath>")
        sys.exit(1)
