...
// Gramark engine: `gramark.Playground`, the filesystem-free compiler core
// bundled to browser ESM (ADR D13, via the sibling `playground` sbt module's
// output, copied to `../generated/gramark-engine.mjs`). The in-browser
// preview and the `gramark` CLI run the same `Lr.parse`, the same scanner
// (built from the grammar's own `## Tokens` block), and the same LR tables —
// so they cannot disagree.
// Ported from site/src/lib/gramark-runtime.ts.
object GramarkRuntime:

  @js.native
  @JSImport("./gramark-engine.mjs", "evaluate")
  ...
  /** The table-construction method: all three build from the same automaton; only the table (and
    ...
    */
  type GramarkMethod = String // "Canonical" | "LALR" | "IELR"

  @js.native
  trait GramarkParseResult extends js.Object:
    val success: Boolean
    val message: String
    ...
    /** Every derivation as gramark-cst JSON; more than one entry only when the grammar is ambiguous
  ...
  private def result(
    ...
    js.Dynamic
      ...
      .asInstanceOf[GramarkParseResult]

  @JSExportTopLevel("parseGramarkDocument")
  def parseGramarkDocument(
      source: String,
      inputOverride: js.UndefOr[String] = js.undefined,
      method: js.UndefOr[GramarkMethod] = js.undefined
  ): js.Promise[GramarkParseResult] =
    ...
    js.Promise.resolve[GramarkParseResult](res)
  ...
  // fence-free `.grmk` projection (ADR D36): a `/** */` banner and `///`
