...
This directory is **scaffolding, not the product.** Gramark's real
implementation is the PureScript code under `src/`. Until that can lex,
parse, and check `.grmk.md` files on its own, this small TypeScript/Node
...
- `gramark-check.ts` — `gramark check` and `gramark fmt`. **check** runs
  three independent gates over a `.grmk.md` file: **structure** (canonical
  layout, fence widths, single H1, trailing newline), **drift** (derived
  artifacts match the sidecar `*.grmk.lock`, per-rule hashing for diagrams and
...
- `railroad.ts` — parses an `gramark` block into a Production and renders it as a
  self-contained SVG or a mermaid `flowchart`.
- `gramark-check.test.ts` / `railroad.test.ts` / `analyze.test.ts` —
...
npm run check -- ../examples/calc.grmk.md          # the structure/drift/lint gates
npm test                                            # unit tests
...
1. `Gramark.Check` (PureScript) reproduces all three gates, and
2. ✅ _met_ — the generated `gramark` parser reads `grammar/lr.grmk.md` back to a
   value equal to `Gramark.Bootstrap.bootstrapGrammar` (the self-host test in
   `test/`, `Test.SelfHost`).
...
condition 1 is also met, `gramark check` is the bundled PureScript binary, the
