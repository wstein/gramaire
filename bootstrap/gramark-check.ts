...
/*
 * gramark --check  (TypeScript prototype)
 *
 ...
 *
 * Usage:
 *   node gramark-check.ts <file.grmk.md>                          # check
 *   node gramark-check.ts fmt [--diagrams=sidecar|mermaid] <file>  # format
 */
...
export interface Block {
  readonly info: string; // full info string, e.g. "gramark precedence"
  readonly lang: string; // first word of info, e.g. "gramark"
...
export function grammarHashes(doc: Doc): GrammarHashes {
  ...
  for (const b of doc.blocks) {
    if (b.lang !== "gramark") continue;
    if (b.info === "gramark" && b.nonterminal) {
      ruleHashes[b.nonterminal] = sha256(`lr\n${b.content}`);
    }
    if (
      b.info === "gramark" ||
      b.info === "gramark precedence" ||
      b.info === "gramark tokens"
    ) {
...
export function checkDrift(file: string, doc: Doc): string[] {
  ...
  if (!existsSync(lockPath))
    return [`no lock file (${basename(lockPath)}); run \`gramark fmt\``];
    ...
    if (current !== a.sourceSha256) {
      fails.push(
        `stale ${a.kind}: ${label} was generated from an older version ` +
          `of ${who}; run \`gramark fmt\``,
      );
    }
    if (path && !existsSync(join(dirname(file), path)))
      fails.push(`missing artifact file: ${path}; run \`gramark fmt\``);
  }
  return fails;
}
...
// `gramark fmt`: (re)emit the derived artifacts for a grammar file. In sidecar
...
export function fmt(file: string, doc: Doc, mode: DiagramMode): void {
  ...
  const stem = basename(file).replace(/\.grmk\.md$/, "");
...
const USAGE =
  "usage: gramark-check.ts <file.grmk.md>            # check\n" +
  "       gramark-check.ts fmt [--diagrams=sidecar|mermaid] <file.grmk.md>";
...
export async function main(argv: readonly string[]): Promise<number> {
  ...
  console.log(`gramark --check ${basename(file)}\n`);
  let failed = false;
