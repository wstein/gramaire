...
class LabApiSuite extends munit.FunSuite:
  ...
  test("evaluate: fences reports every ```gramark fence's role and 1-based line span") {
    val resp = LabApi.evaluate(LabRequest(calcMd, None, Method.Canonical))
    assert(resp.buildOk)
    // examples/calc.grmk.md's own ```gramark fences, in document order: Settings, Tokens,
    ...
    // uses a ```text fence, never ```gramark, so it's correctly absent here.
  ...
  test("evaluate: fences is populated even when the grammar notation fails to parse") {
    val resp = LabApi.evaluate(LabRequest(malformedMd, None, Method.Canonical))
    assert(!resp.buildOk)
    // `malformedMd`'s one ```gramark fence ("Foo Bar", no `NL :`) fails Lr.parseWith, but its
  ...
  test("evaluate: fences is empty for a fence-free native .grmk source") {
  ...
  test(
    "evaluate: analysis reports a live conflict-verdict classification, the same as gramark explain-conflict"
  ) {
    ...
    // under canonical LR(1), the same verdict `gramark explain-conflict` would report.
