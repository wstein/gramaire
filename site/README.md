...
- **Shared** — [LiveGrammar.astro](src/components/LiveGrammar.astro) (the
  ...
  ([../site-glue/js/src/main/scala/gramark/site/](../site-glue/js/src/main/scala/gramark/site/)),
  bundled to `src/generated/site-glue.mjs`: `GramarkRuntime` (the engine
  wrapper), `Diagrams` (the railroad renderer, sharing `gramark.Railroad`
  with the CLI's `gramark fmt`), and `LabLink` (the private-by-hash
...
cd site
npm install
npm run build:engine      # bundle the Scala core → src/generated/gramark-engine.mjs
...
- `src/generated/gramark-engine.mjs` — `npm run build:engine`
  (`sbt playground/fullLinkJS` over `gramark.playground.Main`). The thin
...
`gramark-engine.d.ts` / `site-glue.d.ts` in sync.
