// Resolves a `.grmk.md` prose image's relative path (e.g. `diagrams-calc-js/expr.svg`, as written
// in `![Railroad diagram for the Expr rule](diagrams-calc-js/expr.svg)`) to the SVG's actual raw
// markup, so the Grimoire Notebook can render it inline (`dangerouslySetInnerHTML`, the same
...
//
// `import.meta.glob` eagerly bundles every sidecar SVG `gramark fmt --diagrams=sidecar` writes
...
/** The raw SVG markup for a prose image's `src` (relative to its own `.grmk.md` file, inside
