...
export async function buildPaperPdf(
  ...
  // Matches tokens.css's light-theme FIRST/FOLLOW chip palette exactly (grimoireNotebook.css's
  // `.grimoire__output-ff-chips code[data-kind]`), a separate convention from `rrColors` above:
  ...
  // wrap. Mirrors the Notebook/Paper HTML chips (.grimoire__output-ff-chips) so a rule's
