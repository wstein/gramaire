 ...
 * holds (GrimoireNotebookIsland.tsx's reshape effect only calls this with its own guard-2
...
// Section-aware move (GrimoireNotebookIsland.tsx's own `moveBlock`/`CellActions`) — moving a block
...
// rather than importing markdown.ts's own heading-detection itself — GrimoireNotebookIsland.tsx
 ...
 * of stale-offset drift `attributedDiagnostics`/`GrammarCell`'s squiggle math (GrimoireNotebookIsland.tsx) depend on this
 ...
 * ```gramark fence — same restriction as `paperPdf.ts`'s own `%pdf-figure-scale`, and for the
 ...
 *
 * Lives here, not in `paperPdf.ts` or `GrimoireNotebookIsland.tsx`, for the same reason
