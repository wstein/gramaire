// A foldable rendering of the engine's raw gramark-cst JSON (branches are
// `{rule:<production id>, children:[...]}`, leaves are `{token,text}` — see
// `Gramark.Cst.toJson`). `rule` is a bare numeric id, not a name, so callers
// must also pass `prodLhs` (`GramarkParseResult.prodLhs`, indexed the same
...
/** Render a full foldable tree from one `gramark-cst` JSON string. Returns ""
