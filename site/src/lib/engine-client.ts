...
import type { GramarkMethod, GramarkParseResult } from "./gramark-runtime.ts";

type Reply =
  { id: number; result: GramarkParseResult } | { id: number; error: string };

let worker: Worker | null = null;
let nextId = 1;
const pending = new Map<
  number,
  { resolve: (r: GramarkParseResult) => void; reject: (e: unknown) => void }
>();
