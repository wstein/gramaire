...
test("a rule's diagram and FIRST/FOLLOW are grouped as one accessible figure", async ({
  ...
  await gotoNotebookReady(page);
  const output = ruleCellLocator(page).locator(".grimoire__output");
...
// Notebook/PDF-only. Reusing GrammarCell's own .grimoire__output-ff markup keeps every rule's
...
test("a rule's figure in Paper includes its FIRST/FOLLOW chips alongside the diagram", async ({
  ...
  const figure = page.locator(".grimoire__paper-figure", { hasText: "Factor" });
...
test("%paper-font-scale's own line never renders as visible prose, in either Notebook or Paper", async ({
  ...
  await expect(page.locator(".grimoire__prose").first()).toContainText(
    "Real prose.",
  );
  await expect(page.locator(".grimoire__prose").first()).not.toContainText(
    "paper-font-scale",
  ...
  await viewToggleButton(page, "Paper").click();
  await expect(page.locator(".grimoire__paper")).toBeVisible();
  await expect(page.locator(".grimoire__paper")).toContainText("Real prose.");
  await expect(page.locator(".grimoire__paper")).not.toContainText(
    "paper-font-scale",
  );
