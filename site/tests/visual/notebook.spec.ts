...
test("a cell with only a warning is flagged the same way an error is, and its panel row is clickable", async ({
  ...
  const settingsCell = page
    .locator(".grimoire__cell")
    .filter({ has: page.locator(".grimoire__badge--settings") })
    .first();
  ...
  const warnedCell = page.locator(".grimoire__cell--warning");
  await expect(warnedCell).toHaveCount(1);
  await expect(
    warnedCell.locator(".grimoire__cell-error-tag--warning"),
  ).toHaveText("warning");

  // Clicking the tag reveals the panel without opening the cell's editor.
  await warnedCell.locator(".grimoire__cell-error-tag").click();
  await expect(page.locator(".grimoire__diagnostics")).toBeVisible();
...
test("toggling to Source view replaces the per-cell rendering with one editor over the whole raw document", async ({
  ...
  await page.locator(".grimoire__view-toggle").click();
...
test("editing the raw source and toggling back updates the corresponding cell's railroad diagram", async ({
  ...
  await page.locator(".grimoire__view-toggle").click(); // toggle back, commits on blur
  await expect(page.locator(".grimoire__cell")).toHaveCount(5);
...
test("toggling to Source view and back with no edits leaves the document unchanged", async ({
  ...
  const namesBefore = await page
    .locator(".grimoire__cell-name")
    .allTextContents();

  await page.locator(".grimoire__view-toggle").click();
  await page.locator(".grimoire__view-toggle").click();

  await expect(page.locator(".grimoire__cell-name")).toHaveCount(
    namesBefore.length,
  );
  expect(await page.locator(".grimoire__cell-name").allTextContents()).toEqual(
    namesBefore,
  );
});
