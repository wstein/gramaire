...
// the REAL Grimoire Notebook (same GrimoireNotebookIsland component the standalone /notebook
...
// imports GrimoireNotebookIsland for the first time, which is what actually constructs its
...
test("the homepage shows a prerendered static preview and loads no worker/engine bundle before the Notebook button is clicked", async ({
  ...
  await expect(page.locator(".grimoire")).toHaveCount(0);
...
test("clicking the Notebook button mounts the real Grimoire Notebook and only then loads the engine", async ({
  page,
}) => {
  const requests: string[] = [];
  page.on("request", (req) => {
