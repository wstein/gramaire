...
| D36 | Raw .grmk projection       | **`.grmk.md` is the SINGLE source of truth. `.grmk` is a DERIVED, FENCE-FREE projection (`gramark strip`), ANTLR-style: ALL-CAPS lexer + Mixed-case parser rules told apart by case, `%left`/`%right` kept. Prose → `/** */` banner + `///` doc-comment lines holding Markdown (skipped on parse); headings, errors, diagrams & tables dropped. Author only in `.grmk.md`. `parse(strip(x)) == parse(x)`.**                                                                                                                                                                                       | Gramark's differentiator is that the grammar IS lint-clean Markdown rendering as docs — a second editable format is the sync/trust nightmare the bet avoids. But the ecosystem (editors, linguist, generators, review) expects a plain grammar file that does NOT read as Markdown, so a one-way EXPORT serves it. Fence-free and marker-free because the ```gramark fences made `.grmk` look like Markdown and Gramark, like ANTLR, tells lexer from parser rules by case. Prose is preserved as doc comments — a `/** */` banner for the title/intro, `///` per section — carrying the Markdown verbatim (a Sphinx site consumes it via the MyST-Parser extension; no Markdown→RST translation), so the docs travel with the projection; `toFenced` skips them, and only sections carrying a gramark block survive. Rejected: raw as co-canonical / canonical input; reStructuredText `..` comment markers (tried in a since-reverted commit — the delimiters are doc comments, the content stays Markdown); yacc `%%` sections; `$N` positional actions. The reverse (`.grmk` comments → `.grmk.md` prose) is a future scaffold. |
...
| D38 | Bison `.y` interop         | **Shipped. `gramark import <file.y\|.yy>` (`ConvertBison.importBison`) and `gramark emit --backend bison` (`BackendBison`), mirroring the ANTLR converter pair; `gramark import` dispatches by file extension. Precedence reuses D37 (`%left`/`%right`/`%nonassoc` on quoted literals only — a bare token name in one of those lines is dropped and flagged, matching `Table.parsePrecedence`'s own quoted-literal-only grammar); the prologue/epilogue (`%{ … %}`) and `$N` actions are flagged and dropped, exactly as the ANTLR import drops predicates/actions; `%prec` overrides are likewise dropped and flagged (D37 defers per-rule overrides). `%start` reorders the named rule to lead the output, since Gramark has no start declaration of its own. Because Bison's file is not uniformly lexable in one pass the way ANTLR's `.g4` is — the epilogue after the second bare `%%` is arbitrary, unconstrained C — the importer first runs a raw character-level pre-scan to find the two bare `%%` section boundaries (skipping over comments/quoted strings/`%{…%}` blocks), and only tokenizes the declarations and rules sections. Verified via a full round trip (`.y → import → parse → IR (real precedence) → emit → re-import`) reaching a byte-identical fixed point with zero warnings on re-import, plus a golden test (`test/golden/calc.y`).**                                                                                                                                                                       | Bison/yacc is the other grammar lingua franca; a converter widens Gramark's reach and, alongside the ANTLR one, makes cross-format round-tripping possible. It rides existing seams: the IR is the interchange, precedence is already yacc-shaped (D37), and the "skip the untranslatable prelude, flag it" import strategy is proven on ANTLR (D3/D4). Full parity (union types, error productions, `%glr-parser`) is out of scope; the goal is the Core — rules, alternatives, `%`-precedence — round-tripping cleanly.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| D39 | Cross-format comments      | **Shipped for both converter pairs (the ANTLR carry-through landed as a direct follow-up to the Bison-first rollout, closing the gap this row used to flag). A new optional `Rule.doc: Option[String]` field (Core) threads through to `IRNonterminal.comment` (IR stays version 0 — additive, omitted when empty). Populated only via an explicit opt-in (`Lr.docCommentsOf`/`Lr.withDocComments`) — never by `Lr.parseWith`/`parse` themselves, to protect `SelfHostSuite`'s exact-equality bootstrap check. `ConvertBison`'s import attaches each Bison rule's leading `//`/`/* */` comment as its Gramark section's leading prose; `BackendBison.emit` renders `IRNonterminal.comment` back as a leading `/* … */` block comment. `ConvertAntlr`/`BackendAntlr` now mirror this exactly for `.g4`: `lexG4` emits `TLineComment`/`TBlockComment` tokens (each carrying a `blankBefore` flag, same backward blank-line scan as Bison's), a `leadingDoc` helper attributes a comment run to the FOLLOWING rule only when set off by a blank line (otherwise it's the PREVIOUS rule's trailing note), `G4Rule.doc` carries it through `rulesOf`, and `BackendAntlr.emit` renders `IRNonterminal.comment` back as a leading `/* … */` above the parser rule. A rule's doc comment survives the full round trip in EITHER format: import → parse → `withDocComments` → IR → re-export → re-import, unchanged.**                                                                                                                    | Comments died at the IR boundary before this (the IR carried no prose), so every conversion was documentation-lossy. One optional prose field, plus the explicit-opt-in design, lets docs survive `.y`/`.g4` ↔ `.grmk` round trips — the "best-practice conversion" goal — without risking the exact-equality self-host bootstrap test or the byte-stability of existing goldens (the field is omitted whenever absent). Rejected: a second, format-specific comment channel per target (the sync nightmare D36 avoids). ANTLR import/export comments now carry through the same field, same as Bison's (D38's Bison converter was the first consumer; ANTLR's followed, reusing the identical lexer/heuristic shape).                                                                                                                                                                                                                                                                                                                                                                                                                                         |
