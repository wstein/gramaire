...
| D44 | Product rebrand: Gramark → Grimoire | **Full project rebrand from "Gramark" to "Grimoire", planned now, executed before GA (a hard pre-GA gate, not a post-launch cleanup) — see `docs/rebrand-grimoire-plan.md` for the scope inventory (~1,714 occurrences across 184 files, the `gramark.*` Scala package namespace, the native-image CLI binary, the GitHub repo and its non-auto-redirecting Pages URL) and phased sequencing (brand redesign first, then the mechanical package/site/docs rename, then external surfaces). The `.grmk`/`.grmk.md` file extension is a separate, working-assumption-unchanged decision (it reads "grammar+markdown," not literally "gramark") pending explicit sign-off. The new standalone live-editing notebook page ships now, ahead of the full rebrand, under the name **"Grimoire Notebook"** — a feature-level name that needs no further change once the project-level rebrand lands.** | An internal debate (2026-07-04) on scoping "Grimoire" to just the notebook feature versus the whole project concluded the team would have scoped it to the notebook alone — the shipped "Gramark" wordmark/logomark is a load-bearing pun specifically on "gra-**m**-ark" (the grammar+markdown portmanteau) that doesn't transfer to "Grimoire," so a full rebrand means a genuine brand redesign, not a rename, on top of a ~184-file mechanical migration and a GitHub Pages URL that won't auto-redirect. The product decision overrides that recommendation in favor of the fuller, more distinctive story (grammar and grimoire share the Old French root *gramaire* — "a book you write living things into" fits a live notebook well), gated on GA so the migration has a real deadline rather than drifting. |
