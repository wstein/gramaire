...
| D36 | Raw .grmk projection      | **Markdown `.grmk.md` is the SINGLE source of truth. `.grmk` is a DERIVED, FENCE-FREE projection (`grammark strip`), ANTLR-style: ALL-CAPS lexer/token rules and Mixed-case productions told apart by case, `%left`/`%right` kept, errors and prose dropped. Never a canonical/co-canonical INPUT — authoring is only in `.grmk.md`. `parse(strip(x)) == parse(x)` (Test.Strip).**         | Grammark's differentiator is that the grammar IS lint-clean Markdown rendering as docs — a second editable format is the sync/trust nightmare the bet avoids. But the ecosystem (editors, linguist, generators, review) expects a plain grammar file that does NOT read as Markdown, so a one-way EXPORT serves it. Fence-free and marker-free because the ```grammark fences made `.grmk` look like Markdown (the complaint that drove it) and Grammark, like ANTLR, already tells lexer from parser rules by case — so no `%%` sidecar markers are needed. `toFenced` reads it back for the parser. Rejected: raw as co-canonical / canonical input; yacc-style `%%` sections (ANTLR has no analogue).                                                                                                                                                                                |
