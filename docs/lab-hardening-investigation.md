...
backtracks (`core/src/main/scala/gramark/AtnSim.scala`, `Ll.scala`). For an
...
dangling-else, `examples/dangling-else.grmk.md` — it can cause an outright
...
full-context re-simulation fallback; Gramark's port doesn't have one yet
...
3. **Do nothing; document it.** Already done (`docs/playground-spec.md` §9,
   ...
   dangling-else.grmk.md`) — a legitimate stance on its own, but leaves a
...
- **Where:** `Ll.recognize`/`Ll.parse`/`Ll.parseTraced`
  (`core/src/main/scala/gramark/Ll.scala`) — on the `None`/`Left` path only,
...
- **Explicitly not recommended now:** option 1. It's the more "correct"
  ...
  precedence declaration are rare in hand-written grammars, and Gramark's
...
(JavaCC, ANTLR, Coco/R) has error recovery; Gramark doesn't. The wire
format already anticipates it — `IRRecovery(syncTokens: Vector[Int])`
(`core/src/main/scala/gramark/IR.scala:95`), round-tripped by
...
- **Explicitly not recommended now:** option 1. It's real, production-tool
  ...
  scope creep against Gramark Lab's actual niche (teaching/analysis, not a
