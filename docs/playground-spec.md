...
**Grimoire Notebook (shipped).** A standalone page at `/notebook`
(`site/src/pages/notebook.astro` + `site/src/lab/liveDoc/
GrimoireNotebookIsland.tsx`) — deliberately its own page with its own state
...
memory). Named "Grimoire Notebook" per `docs/rebrand-gramaire-plan.md` — the
feature-level name shipped ahead of any project-wide rebrand. The project's
own target name later changed from "Grimoire" to "Gramaire" (collision-risk
...
Linked from the shared site nav (`gramark-topbar.mjs`'s `SECTIONS`/`LABELS`,
...
cursor and a subtle background tint on hover (`.grimoire__cell-rendered`,
the same "hover-tint, no box" pattern `.grimoire__prose` already used).
...
box); either way, an `EditorToolbar` (`GrimoireNotebookIsland.tsx`) appears
...
paragraph — **except** for one specific image: `gramark fmt`'s own
...
- **Fence markers were part of a cell's editable text**: `block.text` used to
  span the FULL fence including its opening ` ```gramark `/closing ` ``` `
...
Also fixed: `.grimoire` had `min-height: 100vh`, growing it to its own full
...
unreachable. Fixed to `height: 100%; min-height: 0` so `.grimoire__body`
...
`gramark-topbar.mjs`'s `tools` slot needed no changes for the relocation
...
`.grimoire__paper` CSS scope, no component changes needed; rule blocks reuse
...
render them (a labeled `<pre>` of the raw text, `.grimoire__paper-source`),
...
loaded the font — confirmed true for both `GrimoireNotebookIsland.tsx` and
`LabIsland.tsx` (`dangerouslySetInnerHTML`, sharing `page-head.mjs`'s
stylesheet). The CLI's committed sidecar SVGs (`gramark fmt
...
REAL `GrimoireNotebookIsland` (the same component `src/pages/notebook.astro`
...
`site/scripts/prerender-notebook.mjs` runs `gramarkLabEvaluate` — the exact
...
as `GrimoireNotebookIsland`'s new `initial` prop.
...
degrades gracefully to `initial={undefined}` — `GrimoireNotebookIsland`
then behaves exactly as `/notebook` always has (loading placeholder,
evaluate-on-mount).

`GrimoireNotebookIsland`'s new `initial` prop is seeded synchronously on the
...
- **FIRST/FOLLOW as per-rule chips, not a plain-text set.** A rule's own
  ...
  (`.grimoire__output-ff-chips code`, `GrimoireNotebookIsland.tsx`), reusing
  ...
  artifact `gramark fmt` commits to source control; the chips are the
