...
### Rule/token termination and line continuation inside `gramark` blocks (Option A)
...
`gramark` productions block a **line break inside an alternative is
...
anywhere it's rendered. `gramark --check` recomputes and compares against the
lock.
...
exactly the shape `gramark strip` produces from a `.grmk.md`, and `Lr.parse`
