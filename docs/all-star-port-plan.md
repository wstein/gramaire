...
- **Test (`Test.Atn`) →
  ...
  [`AtnInvariantSuite.scala`](../core/.jvm/src/test/scala/gramark/AtnInvariantSuite.scala)
...
- ✅ **A TS/interpreter backend — v1, recognizer-only.** `gramark emit
  ...
  **New test infrastructure**, since no test anywhere in this repo executed
  ...
  `gramark.lab.LabParityMain`'s own architecture) emits the `.ts` for every
...
- ✅ **Phase 3: the execute-and-verify parity gate that automates the
  ...
  mirroring `gramark.Cst.render` (duplicated, not imported — `codegen-scratch`
  must never depend on `gramark-core`). `site/scripts/check-scala-peg-
...
- ✅ **A second opt-in backend, `scala-peg-combinators`, targeting
  ...
  `private[gramark]`) rather than duplicating the `IRProv`-translation logic
...
- ✅ **Two pre-existing engine costs, surfaced (not caused) by this phase,
  ...
  (`core/src/main/scala/gramark/Parser.scala`) used to recompute
  ...
  (`core/src/main/scala/gramark/Ll.scala`) used to recurse
