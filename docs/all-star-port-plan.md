...
- ✅ **Import** (`gramark import <g.g4>` → `.grmk.md`, `Gramark.Convert.Antlr`):
  ...
  Gramark's regex sublanguage) — and **flags** what does not (predicates,
...
- ✅ **Test (`Test.Convert.Antlr`):** a small ANTLR grammar imports to a parsing
  `.grmk.md`; the round trip `import → parse → IR → emit antlr → import` reaches
  a **fixed point**; predicates/actions are flagged and never leak into output.

### Phase 4 — Adaptive lexer (optional, gated) ✅ simulator

- ✅ `Gramark.Lexer.Atn` ports `LexerATNSimulator`: every token class and
...
- ✅ **Test (`Test.LexerAtn`):** the ATN lexer tokenizes **identically** to the
  regex-DFA scanner (`Gramark.Scanner`) on the capture-free `calc`/`json` corpus
